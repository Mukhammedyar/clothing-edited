...
export const productFetching = async ({type, data, id}: dataFetchingType): Promise<any> => {
    switch (type) {
        case 'get':
            return await axios.get(apiUrl + "underwear/get-all")
        case 'create':
            return await axios.post(apiUrl + "underwear/create", data)
        case 'delete':
            return await axios.post(apiUrl +"underwear/delete-one/"+ id)
        case 'edit':
            return await axios.put(apiUrl +"underwear/edit/"+id, data)
        default:
            throw new Error("Invalid type");
    }
}